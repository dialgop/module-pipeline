...
![Pipeline output: detected people outlined in red, a segmented silhouette in green, and the sampled average color shown bottom-left](docs/images/pipeline_output.png)

*Real output from the pipeline running on `external/park.mp4`: `MovementDetector` flags moving regions, `ObjectDetector` verifies which ones are actually people (red boxes), `SegmentationProcessor` outlines a precise silhouette (green), and `ColorProcessor` samples its average color (swatch, bottom-left).*
...
| `SegmentationProcessor` | Refines the motion mask into a precise silhouette of the moving object via contours (see [Segmentation and color strategy](#segmentation-and-color-strategy)). Not yet wired to `ObjectDetector`'s output — see that section for why. |
...
4. **Segmented silhouette** — `SegmentationProcessor`'s refinement of the raw motion mask: just the single largest contour, filled and cleaned up (independent of stage 3, for now).
5. **Overlay** — the segmentation contour and detected-person boxes drawn back onto the original frame, plus the color `ColorProcessor` sampled from the silhouette.
